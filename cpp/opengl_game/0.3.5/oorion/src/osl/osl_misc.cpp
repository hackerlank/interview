...
extern char* OSL_CURRENT_FILE;

void osl_Error(string err_code)
{
	char buffer[192];

	sprintf_s(buffer, 192, "Error (%s, line %u): %s", OSL_CURRENT_FILE,
		OSL_CURRENT_LINE, err_code.c_str());
...
void osl_Error(string err_code, string hint)
{
	...
	sprintf_s(buffer, 192, "Error (%s, line %u): %s (%s)", OSL_CURRENT_FILE,
		OSL_CURRENT_LINE, err_code.c_str(), hint.c_str());
...
void osl_Warning(string warn_code)
{
	...
	sprintf_s(buffer, 192, "Warning (%s, line %u): %s", OSL_CURRENT_FILE,
		OSL_CURRENT_LINE, warn_code.c_str());
