...
public:
	...
	void SetAction(ACTION new_action)	{		action = new_action;	}
	ACTION GetAction() { return action; }
	void SetTexture(texture_t* new_tex)	{		tex = new_tex;	}
	texture_t* GetTexture() const { return tex; }
	void SetCaptionFont(font_t* new_font)	{		captionFont = new_font;	}
	font_t* GetCaptionFont() const { return captionFont; }
	void SetTextFont(font_t* new_font)	{		textFont = new_font;	}
	...
	void SetTextPos(const int new_x, const int new_y)	{		text_x = new_x;		text_y = new_y;	}
	void SetCaptionPos(const int new_x, const int new_y)	{		caption_x = new_x;		caption_y = new_y;	}
	void SetPatternCount(const int count)	{		patternCount = count;	}
	int GetPatternCount() const { return patternCount; }
	void SetCurrentPattern(const int index)	{		currentPattern = index;	}
	int GetCurrentPattern() const { return currentPattern; }
	void SetPatternWidth(const int new_width)	{		patternWidth = new_width;	}
	void SetPatternHeight(const int new_height)	{		patternHeight = new_height;	}
	...
	void SetParent(GUIcontrol* new_parent)	{		parent = new_parent;	}
	void SetParentName(LPSTR new_name)	{		parent_name = new_name;	}
	GUIcontrol* GetParent()	{ return parent; }
	LPSTR GetParentName() { return parent_name; }
	void SetNextName(LPSTR new_name)	{		if(new_name)			next_name = new_name;	}
	void SetNext(GUIcontrol* new_next)	{		if(new_next)			next = new_next;	}
	GUIcontrol* GetNext() { return next; }
	LPSTR GetNextName() { return next_name; }
	void Close()	{		if(parent) parent->KillSelf();	}
	void ExitProgram()	{		current_state = GAME_EXITING;	}
	void SendMessage(GUIMessage message)	{		if(guima) guima->Add(message);	}
	void SetClose(bool flag) { _close = flag; }
	GROUP GetGroup() const { return group; }
	void SetGroup(GROUP new_group) { group = new_group; }
	...
	virtual void Draw()
	{
		...
		if(visible)
		{
			...
			if(textFont && _text)
			{
				...
				TextOut(string(_caption), textFont, &hDC, px, py);
				MessageBox(0,_caption,0,0);
			}
			...
			if(captionFont && _caption)
			{
				...
				TextOut(string(_caption), DEBUG_FONT, &hDC, px, py);
			}
		}
	}
