...
bool LoadTexture(const char file_name[], texture_t* dest, int alphaRed, int alphaGreen, int alphaBlue)
{
	...
	if(ext == "bmp")
	{
		...
		SetTransparent(image, alphaRed, alphaGreen, alphaBlue, 100);
				 ...
				 bool mirrorX, bool mirrorY, UINT scaleX, UINT scaleY)
{
	...
	//	glColor3f(1.0, 1.0, 1.0);
	//	glBegin(GL_QUADS);
	//		glTexCoord2f(float(x1)/tex->width, 1-float(y1)/tex->height); glVertex2f(x, y);
	//		glTexCoord2f((float(x1) + width)/tex->width,1- float(y1)/tex->height); glVertex2f(x + width, y);
	//		glTexCoord2f((float(x1) + width)/tex->width, 1-(float(y1) + height)/tex->height); glVertex2f(x + width, y + height);
	//		glTexCoord2f(float(x1)/tex->width, 1-(float(y1) + height)/tex->height); glVertex2f(x, y + height);
	//	glEnd();
	//	glDisable(GL_TEXTURE_2D);
	//}
...
UINT FindByName(texture_t textures[], char* name)
{
	for(int i = 0; i < tex_count; i++)
	{
		...
		{
			LogToFile("Exception in FindByName()");
