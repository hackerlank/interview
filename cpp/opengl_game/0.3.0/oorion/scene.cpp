...
#include "scene.h"
	...
	unsigned int WINDOW_CAPTION_DY = 0;
	...
	extern UINT race_pick;
	extern UINT size_pick;
	extern UINT difficulty_pick;
	extern UINT opponents_pick;
...
long __stdcall WndProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	//		//if(keys[wParam])
	//		//{
	//			UINT size = pressed_keys.size();

	//			if(size == 0)
	//			{
	//				pressed_keys.push_back(wParam);
	//				prev = GetTickCount();
	//			}
	//			else if(size == 1)
	//			{
	...
	//			else
	//			{
	//				if(GetTickCount() - prev >= KEY_DELAY)
	//				{
	//					if(pressed_keys[size-1] != wParam)
	//					{
	//						pressed_keys.push_back(wParam);
	//						prev = GetTickCount();
	//					}
	...
	//		//		{
	//		//			if(pressed_keys[pressed_keys.size() - 1] != wParam)
	//		//			{
	//		//				pressed_keys.push_back(wParam);
	//		//				prev = GetTickCount();
	//		//			}
	...
	//		int l = pressed_keys.size();
	//		int c = 0;
	//		vector<UCHAR>::iterator it = pressed_keys.begin();
	//		while(c < l)
	//		{
	//			c++;
	//			if((*it) == wParam)
	//			{
	//				std::swap(*it, *(pressed_keys.end()-1));
	//				pressed_keys.pop_back();
	//				unpressed_keys.push_back(wParam);
	//				l--;
	//				c--;
	//			}
	//			else
...
void PrintDebug()
{
	...
	int i = 0;
	PrintText("Keyboard: ", FontByName(DEBUG_FONT), 5, 230);
		...
		it++)
	{
		i++;
		char text = (*it);
		PrintTextSafe(&text, 1, FontByName(DEBUG_FONT), 65+i*15, 230);
	}
#endif
}
...
void ResizeScene(int scr_width, int scr_height, bool fullscreen)
{
	RECT new_rect;
	new_rect.left = 0;
	new_rect.top = 0;
