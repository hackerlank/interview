...
{
	for(UINT i = 0; i < l_sysfunction; i++)
	{
		if(s_sysfunction[i].name == name)
			return (OSL_FUNCTION*)&s_sysfunction[i];
	}
	return NULL;
}
...
bool osl_IsSysFunction(string s)	{ for(UINT i=0;i<l_sysfunction;i++)if(s_sysfunction[i].name==s)return true;return false; }
bool osl_IsSysConstant(string s)	{ for(UINT i=0;i<l_sysconstant;i++)if(s_sysconstant[i].name==s)return true;return false; }
				...
				else if(lexem.type == ttIntConstant || lexem.type == ttFloatConstant)
				{
					if(minus)
					{
						if(osl_IsConstant(prev.token))
						{
							...
							//lexem_stream->push_back(lexem);
							//osl_dDumpLexemStream(lexem_stream);
						}
