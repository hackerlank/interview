...
int GAME::GenerateStar(STAR* dest)
{
	...
	dest->id = count-1;
	dest->owner = NULL;
	dest->x = Random(GetSizeX());
	dest->y = Random(GetSizeY());
...
void GAME::InitStars(int size_pick)
{
	...
	for(int i = 0; i < size_pick * 5 + 4; i++)
	{
		...
		AddStar(*star);
		gui->LogOnScreen("Star " + string(star->name) + " created");
#ifdef _DEBUG_
		LogToFile(string("Created star: name=") + string(star->name));
#endif
	}
}
