	...
	if(tex)
	{
			 ...
			 if (mirrorX==false&&mirrorY==false)
		{
			...
			*/
			...
			//glTexCoord2i(CWidth - (CWidth - x1), CHeight -(CHeight - y1)); glVertex2i(x + x1, y + y1);
			//glTexCoord2i(CWidth - (x1 + x2), CHeight - y1); glVertex2i(x + x1 + x2, y + y1);
			//glTexCoord2i(CWidth -(x1 + x2), CHeight - (y1 + y2)); glVertex2i(x + x1 + x2, y + y1 + y2);
			//glTexCoord2i(CWidth - x1, CHeight - (y1 + y2)); glVertex2i(x + x1, y + y1 + y2);

				  //glTexCoord2f(float(x1)/CWidth, 1-float(CHeight-y2)/CHeight); glVertex2i(x, y + y1);

				  //glTexCoord2f(1-float(CWidth-x2)/CWidth, 1-float(CHeight-y2)/CHeight); glVertex2i(x + (x2-x1)*scaleX/100, y + y1);
