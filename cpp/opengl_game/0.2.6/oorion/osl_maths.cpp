...
bool osl_ABoperand(LEXEM A, LEXEM B, LEXEM op, LEXEM& result)
{
	...
	WriteToLog(DEFAULT_OSL_LOG_NAME, "Result: " + result.token);
	//WriteToLog(DEFAULT_OSL_LOG_NAME, "finished osl_ABoperand()");
	return true; // not sure
}	

bool osl_EvaluateReversePolishNotation(vector<LEXEM>* polish_notation, OSL_VAR* var, vector<OSL_VAR>* pool)
{
	//WriteToLog(DEFAULT_OSL_LOG_NAME, "start osl_EvaluateReversePolishNotation()");
	vector<LEXEM> stack;
	...
	//WriteToLog(DEFAULT_OSL_LOG_NAME, "finished osl_EvaluateReversePolishNotation()");
	return true;
}	
