				...
				vector<INT_VAR>* int_pool, vector<FLOAT_VAR>* float_pool, vector<STR_VAR>* str_pool)
{
	...
	case vtInt:
		{
			int_result = osl_GetIntVar(result_var, int_pool)->value;
			if(IsIntNumber(operand_var))
			{
				int_result = int_result + atoi(operand_var.c_str());
				//int_result += atoi(operand_var.c_str()) + 1; // hui znaet zachem tut 1
	...
	case vtFloat:
		{
			float_result = osl_GetFloatVar(result_var, float_pool)->value;
			if(IsIntNumber(operand_var))
			{
				float_result += (float)atoi(operand_var.c_str());
			}
	...
	case vtStr:
		{
			str_result = osl_GetStrVar(result_var, str_pool)->value;
			if(osl_IsVariable(operand_var))
			{
				VAR_TYPE this_var_type = osl_GetVarType(operand_var, int_pool, float_pool, str_pool);
	...
	case vtInt:
		{
			osl_SetIntValue(int_result, result_var, int_pool);
			break;
		}
	case vtFloat:
		{
			osl_SetFloatValue(float_result, result_var, float_pool);
			break;
		}
	case vtStr:
		{
			osl_SetStrValue(str_result, result_var, str_pool);
			break;
		}
	default: break;
	}
