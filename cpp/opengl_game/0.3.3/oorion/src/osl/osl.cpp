...
bool osl_Load_v_04(const char* file_name)
{
	...
	WriteToLog(DEFAULT_OSL_LOG_NAME, "Loading file: " + string(file_name));
	...
	{
		WriteToLog(DEFAULT_OSL_LOG_NAME, string(file_name) + ": Input FAILED");
		return false;
	}
	...
	{
		WriteToLog(DEFAULT_OSL_LOG_NAME, string(file_name) + ": Lexemize FAILED");
		return false;
	}
	...
	//WriteToLog(DEFAULT_OSL_LOG_NAME, "Compile...");
	//ok = osl_Compile(&lxm, &pool, &storage);

	//if(ok)
	//	WriteToLog(DEFAULT_OSL_LOG_NAME, "Compile OK");
	//else
	//{
	...
	{
		WriteToLog(DEFAULT_OSL_LOG_NAME, string(file_name) + ": Interpret FAILED");
		return false;
	}
