...
void CreatePlayer(string p1, string p2, string p3, string p4,
				  ...
				  string p29, string p30, string p31)
{
	if(iichan)
	{
		if(iichan->GetPlayer())
		...
		if(!(osl_IsIntConstant(p1) &&  osl_IsIntConstant(p2) &&
			...
			osl_IsIntConstant(p29) && osl_IsIntConstant(p30) &&
			osl_IsIntConstant(p31)))
		{
		...
		else
		{
			...
			int jump_p = osl_StrToInt(p27);
			int jump_c = osl_StrToInt(p28);
			int jump_d = osl_StrToInt(p29);
			ANIMATION_TYPE type = ANIMATION_TYPE(osl_StrToInt(p30));
			bool looped = osl_StrToInt(p31) != 0;
			...
			player->SetJumpPattern(jump_p);
			player->SetJumpCount(jump_c);
			player->SetJumpDownPattern(jump_d);
...
void CreateSprite(string p1, string p2, string p3, string p4, string p5)
{
	if(iichan)
	{
		int x = osl_StrToInt(p2);
		int y = osl_StrToInt(p3);
		int w = osl_StrToInt(p4);
		int h = osl_StrToInt(p5);
		texture_t* tex = TextureByName((char*)osl_RemoveQuotesStr(p1).c_str());
		if(!tex)
			osl_Warning("Texture not found");
		...
		spr->SetY(y);
		spr->SetTexture(tex);
		spr->SetPatternWidth(w);
		spr->SetPatternHeight(h);
		spr->SetMirror(false);
		iichan->AddBackground(spr);
	}
	else
	{
		osl_Error("Game is not created");
