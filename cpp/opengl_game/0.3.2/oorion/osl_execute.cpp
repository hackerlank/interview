...
string osl_ExecuteSystemFunction(vector<OSL_LEXEM>* list)
{
	...
	else if(first.token == "createplayer")
	{
		if(param_count == 31)
		{
			...
			vector<OSL_LEXEM>::iterator p29 = list->begin()+58;
			vector<OSL_LEXEM>::iterator p30 = list->begin()+60;
			vector<OSL_LEXEM>::iterator p31 = list->begin()+62;
			...
			CreatePlayer((*p1).token, (*p2).token, (*p3).token, (*p4).token,
				...
				(*p25).token, (*p26).token, (*p27).token, (*p28).token, (*p29).token,
				(*p30).token, (*p31).token);
