...
void PrintDebug()
{
	...
	TextOut("Average flip time: " + string(flip), &fonts[0], &hDC, 5, 130);
		...
		if(OLOLOL) TextOut("lolol", &fonts[0], &hDC, 5, 180);
#endif
...
void InitGUI()
{
	...
	GUIcontrol* diffback = new GUIcontrol("Back");
	...
	gui->difficultymenu->AddChild(diffback);
		...
		it++)
	{
		...
		if((*it)->name == "Back")
		{
			(*it)->SetPrevious(gui->mainmenu);
			(*it)->SetAction(actOpenPrevious);
		}
	...
	GUIcontrol* sizeback = new GUIcontrol("Back");
	...
	gui->sizemenu->AddChild(sizeback);
		...
		it++)
	{
		...
		if((*it)->name == "Back")
		{
			(*it)->SetPrevious(gui->difficultymenu);
			(*it)->SetAction(actOpenPrevious);
		}
	}
	...
	GUIcontrol* opp1 = new GUIcontrol("One opponent");
	GUIcontrol* opp2 = new GUIcontrol("Two opponents");
	GUIcontrol* opp3 = new GUIcontrol("Three opponents");
	GUIcontrol* opp4 = new GUIcontrol("Four opponents");
	GUIcontrol* opp5 = new GUIcontrol("Five opponents");
	GUIcontrol* opp6 = new GUIcontrol("Six opponents");
	GUIcontrol* opp7 = new GUIcontrol("Seven opponents");
	GUIcontrol* opp8 = new GUIcontrol("Eight opponents");
	GUIcontrol* opp9 = new GUIcontrol("Nine opponents");
	GUIcontrol* oppback = new GUIcontrol("Back");

	gui->opponentsmenu->AddChild(opp1);
	gui->opponentsmenu->AddChild(opp2);
	gui->opponentsmenu->AddChild(opp3);
	gui->opponentsmenu->AddChild(opp4);
	gui->opponentsmenu->AddChild(opp5);
	gui->opponentsmenu->AddChild(opp6);
	gui->opponentsmenu->AddChild(opp7);
	gui->opponentsmenu->AddChild(opp8);
	gui->opponentsmenu->AddChild(opp9);
	gui->opponentsmenu->AddChild(oppback);
		...
		it++)
	{
		i++;
		(*it)->SetTexture(&textures[FindByName(textures, "button_long")]);
		(*it)->SetPos(100, 150 + i * 40);
		if(i > 4) (*it)->SetPos(330, 150 + (i-5)*40);
		(*it)->SetPatternCount(3);
		(*it)->SetPatternWidth(220);
		(*it)->SetPatternHeight(30);
		(*it)->SetCurrentPattern(0);
	...
	GUIcontrol* meklar = new GUIcontrol("Meklar");
	GUIcontrol* darlok = new GUIcontrol("Darlok");
	GUIcontrol* human = new GUIcontrol("Human");
	GUIcontrol* silicoids = new GUIcontrol("Silicoids");
	GUIcontrol* mrrshan = new GUIcontrol("Mrrshan");
	GUIcontrol* sakkra = new GUIcontrol("Sakkra");
	GUIcontrol* psilon = new GUIcontrol("Psilon");
	GUIcontrol* klackon = new GUIcontrol("Klackon");
	GUIcontrol* raceback = new GUIcontrol("Back");
		...
		it++)
	{
		...
		if((*it)->name == "Back")
		{
			(*it)->SetPrevious(gui->opponentsmenu);
			(*it)->SetAction(actOpenPrevious);
		}
	}
	...
	//GUIcontrol* progressbar = new GUIcontrol("Progress");

	//gui->loader->AddChild(progressbar);
	//progressbar->SetTexture(&textures[FindByName(textures, "progressbar")]);
	//progressbar->SetPos(50, 50);
	//progressbar->SetPatternCount(1);
	//progressbar->SetPatternWidth(300);
	//progressbar->SetPatternHeight(30);
	//progressbar->SetCurrentPattern(0);
	//progressbar->SetVisible(false);
	//progressbar->SetColor(255, 255, 255, 200);

	//progressbar->SetCaptionFont(&fonts[0]);
	//progressbar->SetCaptionColor(255, 255, 255, 255);
	//progressbar->SetCaptionPos(30, 7);
	//progressbar->SetCaption(gui->loader->name);
