...
void osl_Error(string err_code)
{
	...
	sprintf_s(buffer, 192, "Error (line %u): %s", OSL_CURRENT_LINE, err_code.c_str());
...
void osl_Error(string err_code, string hint)
{
	...
	sprintf_s(buffer, 192, "Error (line %u): %s (%s)", OSL_CURRENT_LINE, err_code.c_str(), hint.c_str());
	WriteToLog(DEFAULT_OSL_LOG_NAME, string(buffer));
}
...
void osl_Warning(string warn_code)
{
	...
	sprintf_s(buffer, 192, "Warning (line %u): %s", OSL_CURRENT_LINE, warn_code.c_str());
	WriteToLog(DEFAULT_OSL_LOG_NAME, string(buffer));
}
