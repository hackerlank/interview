...
// Это попытка чуть переписать интерпретацию для гибкости и поддержки новых конструкций
// Писать постараюсь не торопясь

// Я думаю что надо разбирать интерпретацию на некие состояния
// Например выполнение_функции, итерация_цикла, етц
// Сделать стек таких состояний и таким образом, перемещаясь по коду выполнять то что находится
// на вершине стека

// Тип состояние интерпретации
...
struct OSL_INTERPRET_STATE {
	E_OSL_INTERPRET_STATE type; // тип
	int length; // длительность
};
		...
		case ttOperator:
			{
				...
				{
					// Увеличиваем длительность предыдущего состояния
					top_state.length++;
					state_vector.pop_back();
					state_vector.push_back(top_state);
				}
...
/*
	...
	// Большой стек, в него кладем все лексемы из list
	stack<OSL_LEXEM> big_stack;

	// Стек состояний интерпретации
		...
		it++)
	{
		...
		case ttRightBrace:
			{
				// Маленький стек (одна сентенция)
				stack<OSL_LEXEM> small_stack;
				// Математический вектор для вычисления математики
				vector<OSL_LEXEM> math_vector;

				// Кладем эту { или ; в маленький стек
				OSL_LEXEM colon = big_stack.top();
				big_stack.pop();
				small_stack.push(colon);

				// Заполняем маленький стек (до ближайшей { или ;, или конца стека)		
				bool flag = false;
				while(!flag)
				{
						...
						if(osl_IsOperator(slx.token))
						{
							...
							if(type == optAssign)
							{
								...
								if(osl_EvaluateMath(&math_vector, &temp, pool))
								{
									...
									small_stack.push(var); // кидаем переменную
									math_vector.clear();							
								}
							}
							else
