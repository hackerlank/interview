...
// Очищает пул переменных
void osl_FreeAllVar(vector<OSL_VAR>* pool)
{
#ifdef _DEBUG_OSL
	WriteToLog(DEFAULT_OSL_LOG_NAME, "Undeclaring all variables");
#endif
...
bool osl_CompareConstants(E_OSL_VAR_TYPE t, E_TOKEN_TYPE c)
{
...
void ZeroVarData(OSL_VAR* var)
{
	...
	switch(var->type)
	{
	case vtInt: var->data.int_value = 0; break;
	case vtFloat: var->data.float_value = 0.0f; break;
	case vtStr: var->data.str_value = NULL; break;
	}
}
