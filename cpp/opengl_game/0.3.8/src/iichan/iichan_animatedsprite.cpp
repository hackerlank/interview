...
void ANIMATEDSPRITE::SetCurrentAnimation(enum ANIMATION_TYPE type, bool looped)
{
	if(GetCurrentAnimation() != type)
	{
		...
		case animShoot:
			...
			SetRealX(GetRealShootX());
			SetRealY(GetRealShootY());
			SetRealMirrorX(GetRealShootMirrorX());
			SetRealMirrorY(GetRealShootMirrorY());
			SetRealWidth(GetRealShootWidth());
			SetRealHeight(GetRealShootHeight());
			break;
		case animIdle:
...
void ANIMATEDSPRITE::Draw()
{
	...
	if(GetBlend() > 0.0f)
	{
		...
		glColor4f(current_color[0], current_color[1], current_color[2], 1.0f - GetBlend());
	}
	else
		glDisable(GL_BLEND);
...
void ANIMATEDSPRITE::Move()
{
	...
	if(GetPatternCount() > 0)
	{
		...
		//		if((GetCurrentPattern() == GetRealPatternCount() - 1 ))
				if((GetCurrentPattern() == GetRealPatternCount()))
				{
					SetCurrentPattern(GetStartPattern());
				}
				else
...
void ANIMATEDSPRITE::LoadFromResource(RESOURCE* res)
{
	if(res)
	{
		...
		SetShootAttackPattern(res->shoot_attack_pattern);
		...
		SetRealShootX(res->real_shoot_x);
		SetRealShootY(res->real_shoot_y);
		SetRealShootWidth(res->real_shoot_width);
		SetRealShootHeight(res->real_shoot_height);
		...
		SetRealShootMirrorX(res->real_shoot_mirror_x);
		SetRealShootMirrorY(res->real_shoot_mirror_y);
