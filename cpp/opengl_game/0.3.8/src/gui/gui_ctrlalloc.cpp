...
{
	GID prev_gid = GetFocusedControl();
	GUIBaseControl* prev = GetControlByGID(prev_gid);
	if(prev)
		prev->SetFocus(false);
...
void GUIControlAllocator::HideGroup(int index)
{
	for(vector<GUIBaseControl*>::iterator it = controls.begin();
		it != controls.end();
		it++)
	{
		GUIBaseControl* ctrl = *it;
		if(ctrl->GetGroup() == index)
			ctrl->SetVisible(false);
	}	
}

void GUIControlAllocator::ShowGroup(int index)
{
	for(vector<GUIBaseControl*>::iterator it = controls.begin();
		it != controls.end();
		it++)
	{
		GUIBaseControl* ctrl = *it;
		ctrl->SetVisible(ctrl->GetGroup() == index);
	}
}

void GUIControlAllocator::HideAll()
{
	for(vector<GUIBaseControl*>::iterator it = controls.begin();
		it != controls.end();
		it++)
