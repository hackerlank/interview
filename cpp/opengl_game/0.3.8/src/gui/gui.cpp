...
//}
void GUI::LogOnScreen(string message)
{
	GUIScreenMessage msg;
	msg.time_remains = LOG_SCREEN_TIME;
	msg.message = message;
...
void GUI::wrapperShowGroup(int index)
{
	if(GetControls())
		GetControls()->ShowGroup(index);
}

void GUI::wrapperHideGroup(int index)
{
	if(GetControls())
		GetControls()->HideGroup(index);
}
// --------------------------------------------------------------------------