...
void GUIControl::Process()
{
	...
	if(IsVisible())
	{
		...
		if(IsFocused())
		{
			if(GetTickCount() - prev_key_update >= KEY_DELAY)
			{
				for each(UCHAR key in pressed_keys)
				{
					//SetLastPressedKey(key);
					OnKeyDown(key);
					prev_key_update = GetTickCount();
				}

				for each(UCHAR key in unpressed_keys)
				{
					//SetLastUnpressedKey(key);
					OnKeyUp(key);
				}
			}
		}
...
void GUIControl::OnKeyDown(UCHAR key)
{
	if(sgns.size() > 0)
	{
...
GUIControl::GUIControl(string name)
{
	...
	SetSelectedCaptionColor(DEFAULT_TEXT_COLOR);
	SetPressedCaptionColor(DEFAULT_TEXT_COLOR);
	SetCurrentCaptionColor(DEFAULT_TEXT_COLOR);
	SetCaptionX(0);
	SetCaptionY(0);
	sgns.resize(0);
	SetName(name);
