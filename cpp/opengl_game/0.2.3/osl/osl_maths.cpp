...
bool osl_StackIsLowerPrior(vector<LEXEM>* stack, int prior)
{
	if(!stack)
	{
...
bool osl_Aoperand(LEXEM A, LEXEM op, LEXEM& result)
{
	char operand = op.token[0];

	WriteToLog(DEFAULT_LOG_NAME, "Trying to math: " + A.token + op.token);
	...
	WriteToLog(DEFAULT_LOG_NAME, "Result: " + result.token);
	return true;
}
...
{
	//WriteToLog(DEFAULT_LOG_NAME, "start osl_ABoperand()");
	char operand = op.token[0];

	WriteToLog(DEFAULT_LOG_NAME, "Trying to math: " + A.token + op.token + B.token);
	...
	WriteToLog(DEFAULT_LOG_NAME, "Result: " + result.token);
	//WriteToLog(DEFAULT_LOG_NAME, "finished osl_ABoperand()");
	return true; // not sure
}	

bool osl_EvaluateReversePolishNotation(vector<LEXEM>* polish_notation, OSL_VAR* var, vector<OSL_VAR>* pool)
{
	//WriteToLog(DEFAULT_LOG_NAME, "start osl_EvaluateReversePolishNotation()");
	vector<LEXEM> stack;
		...
		it++)
	{
		...
		else if(osl_IsOperator(lx.token) || osl_IsUnar(lx.token[0]))
		{
			...
			else
			{
				try
				{
					vector<LEXEM>::iterator bb = stack.end()-1;
					vector<LEXEM>::iterator aa = stack.end()-2;
	...
	//WriteToLog(DEFAULT_LOG_NAME, "finished osl_EvaluateReversePolishNotation()");
	return true;
}	
