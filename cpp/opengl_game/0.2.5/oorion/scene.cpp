...
void PrintDebug()
{
	...
	PrintText(time_elapsed, DEBUG_FONT, 5, 230);
	char mx[4];
	sprintf(mx, "Mouse X: %u", mouseX);
	PrintText(mx, DEBUG_FONT, 5, 55);
	char my[4];
	sprintf(my, "Mouse Y: %u", mouseY);
	PrintText(my, DEBUG_FONT, 5, 70);
#endif
#ifdef _DEBUG_TIMINGS
	char draw[32];
	char flip[32];
	...
	PrintText(clear, DEBUG_FONT, 5, 85);
	PrintText(mainloop, DEBUG_FONT, 5, 100);
	PrintText(draw, DEBUG_FONT, 5, 115);
	PrintText(flip, DEBUG_FONT, 5, 130);
#endif
#ifdef _DEBUG_GUI
	if(gui)
	{
		char count[32];
		sprintf(count, "gui->queueCount: %u", gui->GetMessages()->GetMessageCount());
		PrintText(count, DEBUG_FONT, 5, 145);
	}
#endif
				...
				if(current_state == GAME_MAINMENU)
					PrintText(VERSION, DEBUG_FONT, VERSION_X, VERSION_Y);
			#else
				if(current_state == GAME_MAINMENU)
					PrintText(VERSION, DEBUG_FONT, VERSION_X, 25);
			#endif
		#else
			#ifdef VERSION_Y
				if(current_state == GAME_MAINMENU)
					PrintText(VERSION, DEBUG_FONT, 5, VERSION_Y);
			#else
				if(current_state == GAME_MAINMENU)
					PrintText(VERSION, DEBUG_FONT, 5, 25);
			#endif
		#endif		
	#endif
#endif
	...
	PrintText(frames_per_second, DEBUG_FONT, 5, 40);
	...
	PrintText(diff_pck, DEBUG_FONT, 5, 170);
	PrintText(size_pck, DEBUG_FONT, 5, 185);
	PrintText(race_pck, DEBUG_FONT, 5, 200);
	PrintText(opp_pck, DEBUG_FONT, 5, 215);
#endif
}
